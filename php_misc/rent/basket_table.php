...
if (isset($_GET['num_hs'])) {
          $number_house=$_GET['num_hs'];
          $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
          $sql = "SELECT * FROM `orders` WHERE number_house = :num_par";
          $st = $conn->prepare( $sql );
          $st->bindValue( ":num_par", $number_house, PDO::PARAM_STR );
          $st->execute();
}elseif (isset($_GET['fnp'])) {
          $fnp=$_GET['fnp'];
          $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
          $sql = "SELECT * FROM `orders` WHERE fnp = :fnp_par";
          $st = $conn->prepare( $sql );
          $st->bindValue( ":fnp_par", $fnp, PDO::PARAM_STR );
          $st->execute();
         }elseif ((!isset($_GET['num_hs'])) or (!isset($_GET['fnp'])) ) {
          $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
